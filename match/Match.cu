...
__global__ void edgeMerge1(int * flagArray,
						   int * edgeCans,
						   int edgeCansSize,
						   int qid1,
						   int qid2,
						   ...
						   int * effectBit)
{
	...
	if(idx >= totalThreadNum)
		return ;
	int edgeId = idx%edgeCansSize;
	int flagId = idx/edgeCansSize;
	int* curFlagId = flagArray + flagId*qsize;
	if(curFlagId[qid1] != -1)
	{
...
__global__ void edgeMerge2(int * flagArray,
						   int * edgeCans,
						   int edgeCansSize,
						   int qid1,
						   int qid2,
						   ...
						   int * ansArea)
{
	int realIdx = blockIdx.x*blockDim.x + threadIdx.x;
	if(realIdx >= totalThreadNum)
		return ;
	int idx = effectBit[realIdx];
	int edgeId = idx%edgeCansSize;
	int flagId = idx/edgeCansSize;
	int* curFlagId = flagArray + flagId*qsize;
	memcpy(ansArea+realIdx*qsize,curFlagId,sizeof(int)*qsize);
...
*/			
	...
	for(int i = 0; i < qsize; i++)
	{
		for(int j = query->inRowOffset[i]; j < query->inRowOffset[i+1]; j ++)
		{
			if(edgeCount == 0)
			{
				cudaMalloc(&d_flagArray, sizeof(int)*qsize*edgeCansSize[0]);
				cudaMemset(d_flagArray, -1, sizeof(int)*qsize*edgeCansSize[0]);
			}
			int totalThreadNum = midResultNum * edgeCansSize[edgeCount];
			printf("the totalNum is %d\n",totalThreadNum);
			int* effectBit =  new int[totalThreadNum];
			int* d_effectBit;
			int* d_edgeCans;
			cudaMalloc(&d_edgeCans, sizeof(int)*edgeCansSize[edgeCount]*2);
			cudaMemcpy((void *)d_edgeCans, (void *)edgeCans[edgeCount], sizeof(int)*edgeCansSize[edgeCount]*2, cudaMemcpyHostToDevice);
			memset(effectBit,0,sizeof(int)*totalThreadNum);
			cudaMalloc(&d_effectBit, sizeof(int)*totalThreadNum);
			cudaMemcpy((void *)d_effectBit, (void *)effectBit, sizeof(int)*totalThreadNum, cudaMemcpyHostToDevice);
			...
			cudaMalloc(&d_ansArea, sizeof(int)*effectiveSize*qsize);
			cudaMalloc(&d_effectBit, sizeof(int)*effectiveSize);
			cudaMemcpy((void *)d_effectBit, (void *)effectBit, sizeof(int)*effectiveSize, cudaMemcpyHostToDevice);
			edgeMerge2<<<(effectiveSize+MAXTHREADPERBLOCK-1)/MAXTHREADPERBLOCK,MAXTHREADPERBLOCK>>>(d_flagArray,
																									...
																									d_ansArea);
			edgeCount ++;
			printf("after edge %d, the effective size is %d\n", edgeCount, effectiveSize);
	...
	cudaMemcpy((void *)h_ansArea, (void *)d_ansArea, sizeof(int)*effectiveSize*qsize, cudaMemcpyDeviceToHost);
	cudaFree(d_ansArea);
	for(int i = 0; i < effectiveSize; i ++)
	{
		int* tmpPtr = h_ansArea + qsize*i;
		for(int j = 0; j < qsize; j ++)
			printf("(%d, %d) ", j, tmpPtr[j]);
		printf("\n");
	}
	delete [] h_ansArea;
	/*
